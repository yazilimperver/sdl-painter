...
int main() {
  ...
  SDL_Window* window = SDL_CreateWindow(
      "SDLPainter — Phase 1 Demo", 900, 700,
  ...
  while (running) {
    ...
    // --- Dolu dikdörtgenler ---
    painter.SetBrush(sdl_painter::Brush(sdl_painter::Color{60, 120, 200, 200}));
    painter.FillRect(30.0f, 70.0f, 180.0f, 100.0f);

    painter.SetBrush(sdl_painter::Brush(sdl_painter::Color{200, 80, 60, 200}));
    ...
    // --- Dolu daire ---
    painter.SetBrush(sdl_painter::Brush(sdl_painter::Color{50, 200, 120, 220}));
    painter.FillCircle(100.0f, 260.0f, 70.0f);

    // --- Çerçeve daire ---
    painter.SetPen(sdl_painter::Pen(sdl_painter::Color{255, 180, 50, 255}, 4.0f));
    painter.DrawCircle(280.0f, 260.0f, 70.0f);

    // --- Dolu elips ---
    painter.SetBrush(sdl_painter::Brush(sdl_painter::Color{180, 80, 220, 200}));
    painter.FillEllipse(480.0f, 260.0f, 110.0f, 55.0f);

    // --- Çerçeve elips ---
    painter.SetPen(sdl_painter::Pen(sdl_painter::Color{100, 220, 255, 255}, 3.0f));
    painter.DrawEllipse(700.0f, 260.0f, 90.0f, 50.0f);

    // --- Kalın çizgiler ---
    painter.SetPen(sdl_painter::Pen(sdl_painter::Color{255, 255, 255, 200}, 5.0f));
    painter.DrawLine(30.0f,  370.0f, 250.0f, 430.0f);
    painter.DrawLine(30.0f,  430.0f, 250.0f, 370.0f);

    // --- Polyline ---
    painter.SetPen(sdl_painter::Pen(sdl_painter::Color{255, 140, 60, 255}, 4.0f));
    ...
    // --- Çerçeve poligon ---
    painter.SetPen(sdl_painter::Pen(sdl_painter::Color{150, 230, 150, 255}, 2.5f));
    painter.DrawPolygon({
        {600.0f, 350.0f},
        {750.0f, 350.0f},
        {750.0f, 420.0f},
    ...
    painter.SetPen(sdl_painter::Pen(sdl_painter::Color{200, 240, 255, 255}, 2.0f));
    ...
    painter.SetBrush(sdl_painter::Brush(sdl_painter::Color{255, 100, 150, 200}));
    ...
    // --- Çok kenarlı düzenli poligon (5-gen) ---
    {
      ...
      for (int32_t i = 0; i < 5; ++i) {
        ...
        pentagon.push_back({650.0f + std::cos(a) * 55.0f,
                            580.0f + std::sin(a) * 55.0f});
      }
      painter.SetBrush(sdl_painter::Brush(sdl_painter::Color{255, 160, 50, 200}));
      painter.FillPolygon(pentagon);
      painter.SetPen(sdl_painter::Pen(sdl_painter::Color{255, 220, 120, 255}, 2.0f));
